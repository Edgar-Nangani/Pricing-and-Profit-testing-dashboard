...
def price_policy(age=30, gender='female', term=5, sa=1500000,
    ...
    def epv_benefits(premium):
        # np.ravel(...)[0] handles both a plain float and the 1-element
        # array scipy.optimize.root passes in during solving — float()
        # alone raises on numpy >= 2.0 for the array case.
    ...
    prob_surv = inforce[term-1]
    epv_surv = SA * prob_surv * (v ** term)
    epv_benefits_total = epv_death + epv_dis + epv_surr + epv_surv
...
def calculate_irr(profit, inforce):
    """
    Solve for the rate r that sets NPV of the (in-force weighted) profit
    signature to zero. Mirrors the NPV of Profit calc in profit_test but
    treats the discount rate as the unknown instead of a fixed input.
    """
    term = len(profit)
    cashflows = np.array(profit) * np.array(inforce)

    def npv(r):
        return np.sum([cashflows[t] / ((1 + r) ** (t + 1)) for t in range(term)])

    # Scan for a bracket where the NPV curve changes sign, since the
    # profit signature here has multiple sign changes (Image 6 shows a
    # -/+/+/+/- pattern), a single fixed bracket can miss the root.
    # Start the scan at -50% rather than -100%: as r -> -1, (1+r)^t -> 0
    # and NPV(r) blows up to +/-infinity, producing a sign flip that is
    # a discounting artifact, not a real IRR.
    from scipy.optimize import brentq
    rates = np.linspace(-0.5, 5.0, 400)
    vals = [npv(r) for r in rates]
    for i in range(len(rates) - 1):
        if vals[i] == 0:
            return rates[i]
        if vals[i] * vals[i + 1] < 0:
            try:
                return brentq(npv, rates[i], rates[i + 1])
            except Exception:
                continue
    return None


def calculate_breakeven(profit, inforce, rdr):
    """
    First policy year at which cumulative discounted profit (at the
    given risk discount rate) turns non-negative and never dips below
    zero again. Returns None if the policy never breaks even.
    """
    z = 1 / (1 + rdr)
    term = len(profit)
    cum = 0.0
    breakeven_year = None
    for t in range(term):
        cum += profit[t] * inforce[t] * (z ** (t + 1))
        if cum >= 0 and breakeven_year is None:
            breakeven_year = t + 1
        elif cum < 0:
            breakeven_year = None  # dipped back below zero, reset
...
def compute_term_results(age, gender_lower, term, sa, init_exp, init_comm,
    ...
    """Bundle pricing + profit test + IRR/break-even for one term, with
    a bounds guard since the mortality table only covers ages 20-70."""
    if age + term - 1 > 70:
        return None
    ...
    irr = calculate_irr(profit['profit'], pricing['inforce'][:term])
    breakeven = calculate_breakeven(profit['profit'], pricing['inforce'][:term], rdr)
    return {'pricing': pricing, 'profit': profit, 'irr': irr, 'breakeven': breakeven}


def create_stat_card(icon, label, value, sub_text="", color="#2563eb"):
    """Rectangular KPI card for figures that aren't naturally a % of a
    goal (Premium, NPV amounts, IRR, break-even year) — replaces the
    circular gauges for those, which were filling to an arbitrary
    value*1.3 max with no real meaning."""
    return f"""
...
def create_circular_progress(value, max_val, label, color, sub_text=""):
    """Create an animated circular progress indicator with 2dp formatting"""
...
def main():
    ...
    # Sidebar - Dark with all white text
    with st.sidebar:
        ...
        # Sum Assured - White background with dark purple text
        sa = st.number_input(
            "Sum Assured (UGX)", 
            min_value=500000, 
            max_value=5000000, 
        ...
        st.markdown('<div class="sidebar-header" style="margin-top:1.5rem;">⚙️ Assumptions</div>', unsafe_allow_html=True)
        
        # White text for expander headers
    ...
    if calculate:
        with st.spinner("Calculating..."):
            ...
            result = compute_term_results(age, gender_lower, term, sa, init_exp, init_comm,
                                           inflation, ren_exp, ren_comm, mgt_charge, rdr, unit_int)
            if result is None:
                st.error(f"Age {age} + {term}-year term exceeds the mortality table's upper bound (age 70). "
                          "Please lower the age or choose the 5-year term.")
                st.stop()
            ...
            # KPI grid: stat cards for absolute figures, a ring only for
            # Profit Margin since that's the one metric that's genuinely
            # a percentage of something.
            margin_color = "#22c55e" if profit['profit_margin'] > 0 else "#ef4444"
            row1 = st.columns(2)
            with row1[0]:
                st.markdown(create_stat_card(
                    "💵", "Annual Premium (UGX)", f"{pricing['premium']:,.0f}",
                    "Payable yearly", "#2563eb"
                ), unsafe_allow_html=True)
            with row1[1]:
                st.markdown(create_circular_progress(
                    profit['profit_margin'] * 100,
                    max(abs(profit['profit_margin']) * 100 * 1.3, 10),
                    "Profit Margin (%)",
                    margin_color,
                    "Profitable" if profit['profit_margin'] > 0 else "Not Profitable"
                ), unsafe_allow_html=True)
            
            row2 = st.columns(2)
            with row2[0]:
                st.markdown(create_stat_card(
                    "🟣", "NPV of Profit (UGX)", f"{profit['npv_profit']:,.0f}",
                    f"@ {rdr*100:.1f}% RDR", "#7c3aed"
                ), unsafe_allow_html=True)
            with row2[1]:
                st.markdown(create_stat_card(
                    "🟡", "NPV of Premium (UGX)", f"{profit['npv_premium']:,.0f}",
                    f"@ {rdr*100:.1f}% RDR", "#eab308"
                ), unsafe_allow_html=True)
            
            st.markdown("---")
            
            # ---------------------------------------------------------
            # 5-Year vs 10-Year Term Comparison
            # ---------------------------------------------------------
            with st.expander("📐 Compare 5-Year vs 10-Year Term", expanded=False):
                ...
                if other_result is None:
                    st.warning(f"The {other_term}-year term isn't available at age {age} "
                               f"(would run past age 70 on the mortality table).")
                else:
                    ...
                    comp_data = {
                        "Metric": ["Annual Premium (UGX)", "Profit Margin (%)", "NPV of Profit (UGX)",
                                   "NPV of Premium (UGX)"],
                        f"{term_a}-Year Term": [
                            f"{res_a['pricing']['premium']:,.2f}",
                            f"{res_a['profit']['profit_margin']*100:.2f}",
                            f"{res_a['profit']['npv_profit']:,.2f}",
                            f"{res_a['profit']['npv_premium']:,.2f}"
                        ],
                        f"{term_b}-Year Term": [
                            f"{res_b['pricing']['premium']:,.2f}",
                            f"{res_b['profit']['profit_margin']*100:.2f}",
                            f"{res_b['profit']['npv_profit']:,.2f}",
                            f"{res_b['profit']['npv_premium']:,.2f}"
                        ]
                    }
                    ...
                    fig_cmp.add_trace(go.Bar(
                        x=["Premium (UGX)", "Profit Margin (%, x10,000)"],
                        y=[res_a['pricing']['premium'], res_a['profit']['profit_margin']*100*10000],
                        name=f"{term_a}-Year", marker_color="#2563eb"
                    ))
                    fig_cmp.add_trace(go.Bar(
                        x=["Premium (UGX)", "Profit Margin (%, x10,000)"],
                        y=[res_b['pricing']['premium'], res_b['profit']['profit_margin']*100*10000],
                        name=f"{term_b}-Year", marker_color="#7c3aed"
                    ))
            ...
            with st.expander(f"📊 Detailed Results - {term}-Year Term", expanded=False):
                ...
                with col1:
                    ...
                    <div style="background: rgba(37, 99, 235, 0.04); border-radius: 12px; padding: 1rem; border: 1px solid rgba(37, 99, 235, 0.08);">
                        <p style="color: #475569; margin: 0.3rem 0;">✅ <strong style="color: #1e293b;">Death Benefit:</strong> Sum Assured</p>
                        <p style="color: #475569; margin: 0.3rem 0;">✅ <strong style="color: #1e293b;">Disability Benefit:</strong> 110% of Fund Value</p>
                        <p style="color: #475569; margin: 0.3rem 0;">✅ <strong style="color: #1e293b;">Surrender Benefit:</strong> 70% of Fund Value (Year 4+)</p>
                    </div>
                    """, unsafe_allow_html=True)
                with col2:
                    st.markdown("""
                    <div style="background: rgba(37, 99, 235, 0.04); border-radius: 12px; padding: 1rem; border: 1px solid rgba(37, 99, 235, 0.08);">
                        <p style="color: #475569; margin: 0.3rem 0;">✅ <strong style="color: #1e293b;">Surrender Penalty:</strong> 30% of Fund Value</p>
                        <p style="color: #475569; margin: 0.3rem 0;">✅ <strong style="color: #1e293b;">Maturity Benefit:</strong> Sum Assured</p>
                        <p style="color: #475569; margin: 0.3rem 0;">✅ <strong style="color: #1e293b;">Management Charge:</strong> 2.5% p.a.</p>
                    </div>
                    """, unsafe_allow_html=True)
                
                # EPV Components
                st.markdown("#### EPV Components")
                epv_data = {
                    "Component": ["Death Benefits", "Disability Benefits", "Surrender Benefits", 
                                  "Survival Benefits", "Total Benefits", "Expenses", "Commissions", "Premiums"],
                    "EPV (UGX)": [
                        pricing['epv_death'], pricing['epv_dis'], pricing['epv_surr'],
                        pricing['epv_surv'], pricing['epv_benefits'], pricing['epv_expenses'],
                        pricing['epv_commissions'], pricing['epv_premium']
                    ]
                }
                df_epv = pd.DataFrame(epv_data)
                st.dataframe(df_epv.style.format({"EPV (UGX)": "{:,.2f}"}).background_gradient(cmap='Blues', subset=['EPV (UGX)']), use_container_width=True)
                
                # Unit Fund Projection
                st.markdown("#### Unit Fund Projection")
                fund_data = {
                    "Year": list(range(1, term+1)),
                    "Allocated Premium": profit['alloc'],
                    "B/O Spread": profit['spread'],
                    "Mgt Fees": profit['mgt_fees'],
                    "Fund at Year End": profit['fund_end']
                }
                df_fund = pd.DataFrame(fund_data)
                st.dataframe(df_fund.style.format({
                    "Allocated Premium": "{:,.2f}",
                    "B/O Spread": "{:,.2f}",
                    "Mgt Fees": "{:,.2f}",
                    "Fund at Year End": "{:,.2f}"
                }).background_gradient(cmap='Greens', subset=['Fund at Year End']), use_container_width=True)
                ...
                fig.update_layout(
                    title=f"Unit Fund Growth - {term}-Year Term",
                    xaxis_title="Policy Year",
                    yaxis_title="Fund Value (UGX)",
                    hovermode='x unified',
                ...
                # Profit Vector
                st.markdown("#### Profit Vector")
                profit_data = {
                    "Year": list(range(1, term+1)),
                    "Profit": profit['profit'],
                    "Unallocated Premium": profit['unalloc'],
                    "Expenses": profit['expenses'],
                    "Commissions": profit['commissions'],
                    "Interest": profit['interest'],
                    "Extra Death Cost": profit['extra_death'],
                    "Extra Disability Cost": profit['extra_disab'],
                    "Surrender Penalty": profit['surrender_penalty'],
                    "Extra Maturity Cost": profit['extra_maturity_cost']
                }
                df_profit = pd.DataFrame(profit_data)
                st.dataframe(df_profit.style.format({
                    "Profit": "{:,.2f}",
                    "Unallocated Premium": "{:,.2f}",
                    "Expenses": "{:,.2f}",
                    "Commissions": "{:,.2f}",
                    "Interest": "{:,.2f}",
                    "Extra Death Cost": "{:,.2f}",
                    "Extra Disability Cost": "{:,.2f}",
                    "Surrender Penalty": "{:,.2f}",
                    "Extra Maturity Cost": "{:,.2f}"
                }).background_gradient(cmap='RdYlGn', subset=['Profit']), use_container_width=True)
                ...
                fig2.add_trace(go.Bar(
                    ...
                    name='Profit',
                    marker_color=colors,
                    text=[f'UGX {p:,.2f}' for p in profit['profit']],
                    textposition='outside'
                ))
                fig2.update_layout(
                    title=f"Annual Profit Vector - {term}-Year Term",
                    xaxis_title="Policy Year",
                    yaxis_title="Profit (UGX)",
                    hovermode='x unified',
                    template='plotly_white',
                    plot_bgcolor='rgba(255,255,255,0.8)',
                    paper_bgcolor='rgba(255,255,255,0)',
                ...
                st.markdown("#### Multiple Decrement Table")
                decrement_data = {
                    "Age": [age + t for t in range(term)],
                    "qd (Death)": pricing['aqd'],
                    "qdi (Disability)": pricing['aqdi'],
                    "qw (Surrender)": pricing['aqw'],
                    "ap (Survival)": pricing['ap'],
                    "In-force": pricing['inforce']
                }
                df_dec = pd.DataFrame(decrement_data)
                st.dataframe(df_dec.style.format({
                    "qd (Death)": "{:.6f}",
                    "qdi (Disability)": "{:.6f}",
                    "qw (Surrender)": "{:.6f}",
                    "ap (Survival)": "{:.6f}",
                    "In-force": "{:.6f}"
                }).background_gradient(cmap='Purples', subset=['In-force']), use_container_width=True)
        ...
        with col1:
            ...
            <div class="glass-card" style="text-align: center;">
                ...
                <h4 style="color: #1e293b;">1. Input Details</h4>
        ...
        with col2:
            ...
            <div class="glass-card" style="text-align: center;">
                ...
                <h4 style="color: #1e293b;">2. Pricing Engine</h4>
        ...
        with col3:
            ...
            <div class="glass-card" style="text-align: center;">
                <div style="font-size: 2.5rem;">📈</div>
                <h4 style="color: #1e293b;">3. Results</h4>
